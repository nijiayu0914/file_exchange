...
func Routes(app *iris.Application, db *gorm.DB,
	...
	users.Post("/login", func(ctx iris.Context){
		userController := controllers.UserController{UserService: userService}
		userController.Login(ctx, redisClient)
	})
	...
	files.Get("/delete_file_forever", func(ctx iris.Context){
		controllers.DeleteFileForever(ctx, ossOperator)
	})
	...
	files.Post("/copy_file", func(ctx iris.Context){
		controllers.CopyFile(ctx, ossOperator)
	})
