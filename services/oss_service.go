...
func(os *OssOperator) DeleteFilesForever(fileUuid string, fileNames []string) (err error) {
	for _, fileName := range fileNames{
		err = os.DeleteFileForever(fileUuid, fileName)
		if err != nil {
			return err
		}
	}
...
func(os *OssOperator) Copy(originFile string,
	destFile string, versionId string) (err error) {
...
func(os *OssOperator) MultipleCopy(copyList []utils.RequestCopy)(failure []utils.RequestCopy, err error) {
	for _, cp := range copyList{
		err = os.Copy(cp.OriginFile, cp.DestFile, cp.VersionId)
		if err != nil{
	...
	if len(failure) == 0{
		...
		return failure, errors.New("复制失败")
	}
}
