	...
	if err != nil{
		...
		if err != nil{
			...
			if err != nil{
				...
				if checked{
					res = utils.Response{Code: iris.StatusOK, Message: "ok", Data: iris.Map{"token": tokenCache}}
					ctx.StatusCode(iris.StatusOK)
				}else{
					token, err := utils.GenerateToken(userRes.UserName, userRes.Password)
					if err != nil{
						...
						if err != nil{
							...
							res = utils.Response{Code: iris.StatusOK, Message: "ok", Data: iris.Map{"token": token}}
							ctx.StatusCode(iris.StatusOK)
						}
					}
				}
...
func (uc *UserController) ChangePassword(ctx iris.Context){
	...
	err = bcrypt.CompareHashAndPassword([]byte(userRes.Password), []byte(changePassword.OldPassword))
	if err != nil {
		ctx.StatusCode(iris.StatusUnauthorized)
		ctx.JSON(utils.Response{Code: iris.StatusUnauthorized, Message: "密码错误"})
		return
	}
	userId, err := uc.UserService.ChangePassword(userRes, changePassword.NewPassword)
	if err != nil{
		ctx.StatusCode(iris.StatusBadRequest)
		ctx.JSON(utils.Response{Code: iris.StatusBadRequest, Message: "密码修改失败"})
		return
	}
	ctx.StatusCode(iris.StatusOK)
